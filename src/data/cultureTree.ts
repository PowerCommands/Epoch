...
/** Completing this node makes the World Council transition available. */
export const ENLIGHTENMENT_CULTURE_NODE_ID = 'enlightenment';

/** Completing this node reveals land archaeological resources to a nation. */
export const HUMANISM_CULTURE_NODE_ID = 'humanism';

/** Completing this node reveals Shipwrecks. */
export const NATURAL_HISTORY_CULTURE_NODE_ID = 'natural_history';
...
/** Whether a culture unlock must be satisfied alongside a building's technology unlock. */
export function isCultureRequiredWithTechnologyForBuilding(buildingId: string): boolean {
  return CULTURE_TREE.some((cultureNode) =>
    (cultureNode.unlocks ?? []).some((unlock) =>
      unlock.type === 'building'
      && unlock.value === buildingId
      && unlock.requiredWithTechnology === true,
    ),
  );
}
