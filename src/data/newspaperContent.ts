...
export const NEWSPAPER_EVENT_DEFINITIONS: Readonly<Record<NewspaperEventType, NewspaperEventDefinition>> = {
  ...
  cityDeveloped: definition(60, NEWSPAPER_IMAGE_PATHS.cityFounded, 'the growth of a new city',
    c => c.event.metadata?.firstCity
      ? `THE WORLD'S FIRST CITY: ${upper(c.cityName ?? 'A NEW URBAN CENTER')}`
      : `${upper(c.cityName ?? 'A SETTLEMENT')} BECOMES A CITY`,
    c => `${c.cityName ?? 'A settlement'} in ${names(c)[0]} has completed its urban development and become ${c.event.metadata?.firstCity ? "the world's first City" : 'a City'}. Streets, homes and infrastructure now form a connected, living urban center. Its population capacity has increased by five.${c.event.metadata?.firstCity ? ' This marks a new chapter in the history of settlement and urban life.' : ''}`),
