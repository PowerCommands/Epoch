...
export const TUTORIAL_SECTIONS: readonly TutorialSection[] = [
    ...
    blocks: [
      ...
      { kind: 'heading', text: 'During Play' },
      {
        ...
        items: [
          ...
          'Left / Right arrows — Cycle through units sharing the selected tile, including aircraft at Airports / Airbases, passengers on transport ships, aircraft on carriers, and other stacked units. Wraps around at either end; the camera stays still while switching units',
          'I — Open the tile information window for the current selection; while it is open it follows your selection and updates as you pick another tile. Click a unit portrait to select that unit and close the window — useful when several units share a tile. Press I again on the same tile to close it',
  ...
  },
  {
    ...
    title: 'Trade',
    blocks: [
      { kind: 'paragraph', text: 'Trade turns exportable surplus into Gold per turn without reducing your own resource access or bonuses. The buyer pays for resource access: luxuries can support Happiness, while manufactured goods can add Gold, Food, Production or Happiness. Routes build Trust and resource deals build Affinity — choosing whom to strengthen is part of geopolitics.' },
      { kind: 'heading', text: 'Creating a Trade Route' },
      {
        kind: 'list',
        items: [
          'Establish mutual embassies and Trade Relations in Diplomacy. Both nations need Foreign Trade; accepting their embassy does not open yours.',
          'Open Trading → Sell, choose a resource, destination city and active duration, then select Sell.',
          'If existing route capacity is available, the deal starts immediately. Otherwise Sell automatically queues a new route in an origin city.',
          'A new route normally needs 10 preparation turns (scenario settings may differ), after earlier production finishes. Income, import benefits and the full deal duration begin on activation.',
        ],
      },
      ...
      { kind: 'paragraph', text: 'Every city starts with one Trade Capacity. Commercial and port buildings add more:' },
      {
        kind: 'list',
        items: [
      ...
      { kind: 'note', text: 'Sell lists eligible destinations. If no destination is available, check Trade Relations, export availability and both cities’ free capacity. Build commercial or port buildings to support more routes. Buy requires an active route with free import capacity.' },
      { kind: 'heading', text: 'In Progress' },
      { kind: 'paragraph', text: 'The resource deal is pending: no Gold is paid and no import bonus is granted yet. Check the origin city production queue. A route behind a continuous project cannot begin until you move it ahead or replace the project. The pending resource is checked again when the route activates.' },
      { kind: 'heading', text: 'Active' },
      { kind: 'paragraph', text: 'The route can carry resource deals. Active exports earn Gold per turn; imports cost Gold and provide resource access and its benefits. Check Trading Overview or the partner tab for active deals, remaining duration and your trade balance. War ends the trading relationship, while embargoes and boycotts can disrupt imports.' },
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: 'paragraph', text: 'To choose among units sharing a tile, use the Left / Right arrow keys. This includes aircraft at Airports and Airbases, cargo aboard transport ships, carrier aircraft and visible covert units sharing a city. You can also press I to open Information and click the portrait of the unit you want; it becomes selected and the window closes.' },
      { kind: 'paragraph', text: 'Land units can board compatible transport ships. Debark always requires an adjacent available land tile, even after industrial-era technology allows independent embarkation. Independently embarked land units appear as a bobbing small boat with a 1 badge, and regain their normal appearance on land.' },
      { kind: 'heading', text: 'Excavating Shipwrecks' },
      { kind: 'paragraph', text: 'Load an Archaeologist aboard any ship that can carry one, move the ship onto a revealed Shipwreck, then select Dig on the ship. Archaeology research is required. Keep the Archaeologist aboard and the ship over the wreck for all four turns. Moving the ship or unloading the Archaeologist interrupts the work. An independently embarked Archaeologist cannot excavate a Shipwreck. A disabled Dig button explains what is missing.' },
