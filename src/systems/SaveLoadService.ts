...
export class SaveLoadService {
  ...
  static serialize(context: SaveLoadContext): SavedGameState {
    ...
    const cities: SavedCity[] = cityManager.getAllCities().map((city) => {
      ...
      return {
        id: city.id,
        settlementStage: city.settlementStage,
        ...
        energyShortageTurns: city.energyShortageTurns,
  ...
  static serializeDiplomacy(diplomacyManager: DiplomacyManager): SavedDiplomacyEntry[] {
    return diplomacyManager.getAllStates().map((entry) => ({
      ...
      economicPressureFromAToB: entry.relation.economicPressureFromAToB,
      economicPressureFromAToBTurn: entry.relation.economicPressureFromAToBTurn,
      economicPressureFromAToBRemovalOfferPresented: entry.relation.economicPressureFromAToBRemovalOfferPresented,
      economicPressureFromBToA: entry.relation.economicPressureFromBToA,
      economicPressureFromBToATurn: entry.relation.economicPressureFromBToATurn,
      economicPressureFromBToARemovalOfferPresented: entry.relation.economicPressureFromBToARemovalOfferPresented,
  ...
  static apply(state: SavedGameState, context: SaveLoadContext): void {
    ...
    context.impulsiveBullySystem?.restore(state.impulsiveBully);
    context.leaderStatementSystem?.restore(state.leaderStatements);
  ...
  private static applyCitiesAndProduction(
    ...
    for (const saved of cities) {
      ...
      const city = cityManager.restoreCity({
        ...
        energyShortageTurns: saved.energyShortageTurns,
  ...
  static restoreDiplomacy(
    ...
    for (const entry of entries ?? []) {
      ...
      diplomacyManager.restoreState(entry.nationA, entry.nationB, {
        ...
        economicPressureFromAToB: entry.economicPressureFromAToB,
        economicPressureFromAToBTurn: entry.economicPressureFromAToBTurn,
        economicPressureFromAToBRemovalOfferPresented: entry.economicPressureFromAToBRemovalOfferPresented,
        economicPressureFromBToA: entry.economicPressureFromBToA,
        economicPressureFromBToATurn: entry.economicPressureFromBToATurn,
        economicPressureFromBToARemovalOfferPresented: entry.economicPressureFromBToARemovalOfferPresented,
        trust: entry.trust,
        fear: entry.fear,
        hostility: entry.hostility,
        affinity: entry.affinity,
