...
export const RESOURCE_AMBIENT: Record<string, AmbientProfile> = {
  ...
  horses: {effects: [], animal:'horses', note:'Continuous gallop in place with gathered and extended legs, suspension, head nod and streaming tail.'},
  ivory: {effects: [], animal:'elephant', note:'Living tusked elephant walking in place in a seamless 2.8-second loop with alternating feet, swaying trunk, ears and tail.'},
...
export const IMPROVEMENT_AMBIENT: Record<string, AmbientProfile> = {
  farm: {effects: [], buildingActivity:'farm', joints: [j(.28,.62,.13,.022,0,'wind'),j(.48,.74,.13,.022,0,'wind'),j(.61,.62,.11,.018,0,'wind')]},
  plantation: {effects: [], joints: [foliage(.52,.19,.095),foliage(.81,.39,.105),foliage(.29,.64,.14)]},
  lumber_mill: {effects: [], buildingActivity:'lumber_mill', joints: [foliage(.25,.26,.17)]},
...
export const BUILDING_AMBIENT: Record<string, AmbientProfile> = {
  dock: {effects:[],buildingActivity:'dock'},
  ...
  garden: {effects:[],buildingActivity:'garden'},
  ...
  oil_power_plant: p(e('smoke',.49,.23,.75,0x93958e)), opera_house: {effects:[],buildingActivity:'opera_house'},
  ...
  container_port: {effects:[],buildingActivity:'container_port',
    note:'Two gantry trolleys hoist containers, traverse the painted booms, lower and release onto the ship, then return empty. Freight trucks use the clear terminal road; small ripples follow the hull. Fixed cranes, dock and ship.'},
...
export const WONDER_AMBIENT: Record<string, AmbientProfile> = {
  ...
  great_wall: {effects:[],buildingActivity:'great_wall'},
  ...
  'taj-mahal': {effects:[],buildingActivity:'taj_mahal'},
...
export const UNIT_AMBIENT: Record<string, AmbientProfile> = {
  ...
  great_war_bomber:p(e('propeller',.232,.535,1,0xb99865,.48),e('propeller',.496,.720,1,0xb99865,.48)),
