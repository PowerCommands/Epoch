...
import { globeDestination, planetaryHalfExtents, planetaryParameters, planetaryStrength, planetaryZoomRange, unprojectPlanetary, type PlanetarySurface, type PlanetaryView } from './PlanetaryProjection';
...
void main() {
    ...
    float z = sqrt(1.0 - r2);
    float ry = p.y * cos(orientation.y) + z * sin(orientation.y);
    float rz = z * cos(orientation.y) - p.y * sin(orientation.y);
    float lon = mod(atan(p.x, rz) + orientation.x + 3.14159265359, 6.28318530718) - 3.14159265359;
    vec2 angles = vec2(lon - orientation.x, asin(clamp(ry, -1.0, 1.0)) - orientation.y);
    vec2 sourceDelta = angles * sourceScale / bend;
    sourceDelta.x -= surfaceShear * sourceDelta.y;
    vec2 uv = (viewport * 0.5 + sourceOffset + sourceDelta) / viewport;
...
class PlanetaryNode extends Phaser.Renderer.WebGL.RenderNodes.BaseFilterShader {
  ...
  setupUniforms(controller: PlanetaryFilter): void {
    ...
    // Filter UVs run bottom-to-top; input/world coordinates run top-to-bottom.
    const destination = v.navigation ? globeDestination(v) : null;
    this.programManager.setUniform('orientation', v.navigation ? [v.navigation.longitude * v.strength, -v.navigation.latitude * v.strength] : [0, 0]);
    this.programManager.setUniform('sourceOffset', destination ? [(destination.x - v.navigation!.centerX) * v.zoom, (v.navigation!.centerY - destination.y) * v.zoom] : [0, 0]);
    this.programManager.setUniform('viewport', [v.width, v.height]);
    this.programManager.setUniform('surfaceShear', params.shear);
    ...
    this.programManager.setUniform('mapRect', [left, 1 - top - v.mapHeight * cam.zoom / v.height, left + v.mapWidth * cam.zoom / v.width, 1 - top]);
  }
}
...
export class PlanetaryRenderer {
  ...
  navigation: { longitude: number; latitude: number } | null = null;
  ...
  constructor(private readonly camera: Phaser.Cameras.Scene2D.Camera, renderer: Phaser.Renderer.WebGL.WebGLRenderer,
    private readonly mapWidth: number, private readonly mapHeight: number, private readonly surface?: PlanetarySurface) {
    ...
    camera.clampX = x => {
      if (this.navigation && this.view.strength > 0) return x;
    ...
    camera.clampY = y => {
      if (this.navigation && this.view.strength > 0) return y;
    ...
    camera.getWorldPoint = <O extends Phaser.Math.Vector2>(x: number, y: number, output = new Phaser.Math.Vector2() as O): O => {
      ...
      if (!point || (this.navigation && (point.x < 0 || point.x > camera.width || point.y < 0 || point.y > camera.height))) { output.x = -1e9; output.y = -1e9; return output; }
  ...
  get view(): PlanetaryView {
    ...
    return { surface: this.surface, width: this.camera.width, height: this.camera.height, mapWidth: this.mapWidth, mapHeight: this.mapHeight,
      navigation: this.navigation ? { ...this.navigation, centerX: this.camera.scrollX + this.camera.width / 2, centerY: this.camera.scrollY + this.camera.height / 2 } : undefined,
