...
export function createAILogFormatter(deps: AILogFormatterDeps): AILogFormatter {
  return function formatLog(nationId: string, message: string): string {
    ...
    const round = deps.turnManager.getCurrentRound();
    const year = deps.turnManager.getGlobalYearLabel();
    return `[r${round}] [${year}] ${nationName} (era: ${era}, gold: ${gold}, happiness: ${happiness}) ${message}`;
  };
}
