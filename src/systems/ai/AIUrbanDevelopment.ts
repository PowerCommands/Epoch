...
import { METROPOLIS_DEVELOPMENT_REQUIREMENTS, CITY_DEVELOPMENT_REQUIREMENTS, getSettlementStage, canDevelopIntoTown, getUrbanSlots, getUrbanRequirement } from '../UrbanDevelopment';
...
export function getUrbanInfrastructureCandidates(
  ...
  if (getSettlementStage(buildings, city) !== 'Village') {
    ...
    for (const options of stage === 'Town' ? CITY_DEVELOPMENT_REQUIREMENTS : stage === 'City' ? METROPOLIS_DEVELOPMENT_REQUIREMENTS : []) {
      if (buildings.getAllEntries().some(entry => options.some(id => buildingFulfillsRequirement(entry.buildingId, id)))) continue;
