...
export function renderSettlementProgress(progress: SettlementProgress): HTMLElement {
  ...
  text('p', `Population capacity +${progress.populationBonus}${!progress.possible ? ' · Unavailable at this location' : progress.stage === progress.to ? ' · Bonus unlocked' : ' when development completes'}`);
  if (!progress.possible) {
    text('p', 'This Village cannot develop into a City at this location. Its founding geography does not support the six required surrounding tiles. Mountains, ocean, or four or more water tiles prevent development. Research and construction cannot remove this restriction.').className = 'development-warning';
  } else {
    text('p', progress.stage === progress.to ? 'City development complete. Further settlement levels are not available yet.' : `${progress.completed} / ${progress.slots.length} buildings complete. Your Village becomes a City automatically when all required buildings are finished.`);
  }
  ...
  if (progress.possible) {
    if (progress.stage !== progress.to) {
      ...
      text('p', progress.missingTechs.length ? 'Missing technologies, including prerequisites: ' + progress.missingTechs.map(t => t.name).join(' · ') : 'All required technologies have been researched.');
    }
  }
  return root;
}
