...
export const ACTIONS: readonly UnitActionDefinition[] = [
  ...
  },
  {
    mode: 'debark',
    label: 'Debark',
    isAvailable: (unit) => unit.unitType.isNaval === true && !unit.unitType.aircraftCapacity && hasCargoCapacity(unit.unitType),
  },
  {
    mode: 'repair',
    label: 'Repair',
...
export class UnitActionToolbox {
  ...
  private getActionLabel(
    ...
    if (action.mode === 'build' && this.selectedUnit) {
      const improvement = this.getBuildPreview(this.selectedUnit).improvement;
      if (improvement) return `Build ${improvement.name}`;
    }
