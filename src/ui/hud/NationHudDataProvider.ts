  ...
  getResourceEntries(nationId: string): HudResourceEntry[] {
    ...
    const entries: HudResourceEntry[] = [
      ...
      },
      {
        ...
        delta: nationResources.effectiveGoldPerTurn,
        upkeep: unitUpkeep,
        tooltip: ['Gold',
          ...(nationResources.mutualFoeGoldOutgoingPerTurn > 0 ? [`Mutual Foe Agreement: -${nationResources.mutualFoeGoldOutgoingPerTurn}`] : []),
          ...(nationResources.mutualFoeGoldIncomingPerTurn > 0 ? [`Mutual Foe Agreement Support: +${nationResources.mutualFoeGoldIncomingPerTurn}`] : []),
        ].join('\n'),
      },
      {
        key: 'influence',
