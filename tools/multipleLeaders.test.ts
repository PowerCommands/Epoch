...
test('England exposes Henry V as default and Winston Churchill as a real alternative', () => {
  ...
  assert.deepEqual(leaders.map((leader) => leader.id), ['leader_henry_v', 'leader_winston_churchill', 'leader_tony_blair', 'leader_boris_johnson']);
...
test('an explicit active leader drives centralized lookup and leader personality', () => {
  ...
  try {
    const expectedIds = ['leader_henry_v', 'leader_winston_churchill', 'leader_tony_blair', 'leader_boris_johnson', alternative.id];
