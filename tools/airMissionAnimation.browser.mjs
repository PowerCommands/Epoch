...
export async function checkAirMissionAnimations(page, url) {
  ...
  for (const canvas of [false, true]) {
      ...
      state.game = new Phaser.Game({ type: canvas ? Phaser.CANVAS : Phaser.WEBGL, width: 900, height: 600,
        backgroundColor: '#55694d', audio: { noAudio: true }, scene: {
          ...
          create() {
            ...
            const map = { tileToWorld: (x,y) => ({ x: 50 + x * 50, y: 50 + y * 50 }),
    ...
    for (const id of ids) {
      ...
      await page.evaluate(() => window.airTest.clean());
    ...
    const lifecycle = await page.evaluate(() => {
      ...
      const fogged = s.scene.children.list.filter(o => o.name !== 'test-ground').every(o => !o.visible || o.type === 'Graphics' && o.commandBuffer.length === 0);
      ...
      const clean = s.scene.children.list.filter(o => o.name !== 'test-ground').length === 0 && s.listeners.size === 0;
