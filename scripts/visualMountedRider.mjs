...
const output=process.env.EPOCH_TEST_OUTPUT??'/tmp/epoch-mounted-rider';
...
await page.evaluate(()=>{
 ...
 for(const [x,size,id] of [[350,550,'horseman'],[850,180,'knight']]) {
  ...
  a.attach(sprite,'unit','riding-preview',()=>[0,0]);
 }
...
for(const phase of [0,.25,.5,.75]) {
 const result=await page.evaluate(phase=>{
  ...
  a.elapsed=phase*1250;a.lastDraw=-Infinity;a.update(0,0);
 ...
 assert.ok(result);
 const capture=await page.screenshot({path:`${output}/rider-${phase}.png`});
 ...
 for(const [x,y,w,h,minimum] of [[180,190,350,540,10000],[790,370,130,170,1000]]) {
  ...
  assert.ok(visible>minimum,`mounted sprite disappeared at phase ${phase}: ${visible} visible pixels`);
 }
}
await page.evaluate(()=>{
...
await page.screenshot({path:`${output}/rider-still.png`});
assert.deepEqual(errors,[]);
console.log('Mounted rider visible in four gait phases, nested circle clips and still mode.');
