...
const output=process.env.EPOCH_TEST_OUTPUT??'/tmp/epoch-classic-units';
...
const ids=['longswordsman','horseman','cavalry'];
await page.evaluate(()=>window.reviewGame.loop.stop());
for(const displaySize of [240,100]) {
 await page.evaluate(([ids,displaySize])=>{
  ...
  ids.forEach((id,i)=>{
   ...
   s.add.text(x,y+150,id,{fontSize:'13px',color:'#fff'}).setOrigin(.5).setDepth(30);
  });
 ...
 for(const phase of [.32,.45,.49,.60,.9]) {
  ...
  assert.ok(checks.every(c=>(c.key==='cavalry'||c.drawing)&&c.finite),JSON.stringify(checks));
  await page.screenshot({path:`${output}/classic-${displaySize}-${phase}.png`});
 }
}
const visibility=await page.evaluate(()=>{
 const a=window.reviewAmbient,s=window.reviewScene;
 s.cameras.main.setZoom(.75);a.lastDraw=-Infinity;a.update(0,0);
 const zoomed=[...a.bindings].every(b=>b.key==='cavalry'||b.drawing);
...
console.log(`Reviewed ${ids.length} classic unit profiles at 240px and 100px in five animation phases.`);
