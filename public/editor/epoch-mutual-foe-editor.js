/* Generated from src/editor/mutualFoeEditorBundle.ts — do not edit. */
"use strict";
(() => {
  ...
  // src/systems/MutualFoeAgreementValidation.ts
  function validateMutualFoeAgreements(input, nations) {
    const agreements = [];
    const errors = [];
    if (input === void 0) return { agreements, errors };
    ...
    for (const [index, item] of input.entries()) {
      ...
      if (!a || typeof a !== "object") {
        errors.push(`Agreement ${index + 1}: invalid definition.`);
        continue;
      }
      ...
      if (!Array.isArray(a.memberLeaderIds) || a.memberLeaderIds.length < 2) problems.push("at least two member leaders are required");
        ...
        const seenLeaders = /* @__PURE__ */ new Set();
        const seenNations = /* @__PURE__ */ new Set();
        for (const id of a.memberLeaderIds) {
          const leader = ALL_LEADERS.find((l) => l.id === id);
          if (typeof id !== "string" || !leader) {
            problems.push(`unknown leader: ${String(id)}`);
            continue;
          }
          ...
          seenLeaders.add(id);
          seenNations.add(leader.nationId);
        }
      }
  ...
  // src/editor/mutualFoeEditorModel.ts
  function mutualFoeLeaderOptions(nations) {
  ...
  function newMutualFoeAgreement(existing) {
    let index = 1;
  ...
  function removeMutualFoeAgreement(agreements, id) {
    return agreements.filter((a) => a.id !== id);
  }

  // src/editor/mutualFoeEditorBundle.ts
  var node = (tag, text) => {
  ...
  function open(scenario, onApply) {
    let drafts = Array.isArray(scenario.mutualFoeAgreements) ? structuredClone(scenario.mutualFoeAgreements).filter((a) => a && typeof a === "object") : [];
    let selected = 0;
    const dialog = node("dialog");
    dialog.className = "mf-dialog";
    dialog.setAttribute("aria-label", "Mutual Foe Agreements");
...
`;
    const nav = node("nav");
    nav.setAttribute("aria-label", "Agreements");
    const main = node("div");
    const errors = node("div");
    errors.className = "mf-errors";
    ...
    function render() {
      ...
      errors.textContent = "";
      drafts.forEach((a2, i) => {
        const b = button(a2.name || a2.id || `Agreement ${i + 1}`, () => {
          selected = i;
          render();
        });
      ...
      nav.append(button("Create Agreement", () => {
        drafts.push(newMutualFoeAgreement(drafts));
        selected = drafts.length - 1;
        render();
      }));
      const a = drafts[selected];
      if (!a) {
        main.append(node("p", "No agreements defined. Create one to select its antagonist and signatory leaders."));
        return;
      }
      ...
      id.value = a.id ?? "";
      id.readOnly = true;
      field("Agreement ID", id);
      const name = node("input");
      name.value = a.name ?? "";
      name.oninput = () => {
        a.name = name.value;
      };
      field("Agreement name", name);
      const antagonist = node("select");
      antagonist.append(new Option("Select antagonist nation\u2026", ""));
      ...
      antagonist.onchange = () => {
        ...
        a.memberLeaderIds = (Array.isArray(a.memberLeaderIds) ? a.memberLeaderIds : []).filter((id2) => allowed.has(id2));
        render();
      };
      ...
      percent.max = "100";
      percent.step = "any";
      percent.value = String(a.supportPercent ?? "");
      percent.oninput = () => {
        a.supportPercent = percent.valueAsNumber;
      };
      field("Support (%)", percent);
      ...
      members.append(node("legend", "Member leaders \u2014 at least two, one per nation"));
      ...
      for (const l of options) {
        ...
        check.type = "checkbox";
        check.checked = selectedIds.includes(l.id);
        check.disabled = !check.checked && selectedIds.some((id2) => options.some((other) => other.id === id2 && other.nationId === l.nationId));
        check.onchange = () => {
          a.memberLeaderIds = check.checked ? [...selectedIds, l.id] : selectedIds.filter((id2) => id2 !== l.id);
          render();
        };
        row.append(check, document.createTextNode(l.label));
        members.append(row);
      }
      for (const id2 of selectedIds.filter((id3) => !options.some((l) => l.id === id3))) {
        members.append(button(`Remove unavailable member: ${id2}`, () => {
          a.memberLeaderIds = selectedIds.filter((value) => value !== id2);
          render();
        }));
      }
      main.append(members, button("Remove Agreement", () => {
        drafts = removeMutualFoeAgreement(drafts, a.id);
