...
(() => {
  ...
  var CULTURE_TREE = [
    ...
    node({ id: "drama_civics", name: "Drama and Poetry", era: "classical", cost: 120, description: "Public performance gives a civilization a voice. Poetry, theatre, and ceremony carry memory from one generation to the next.", prerequisites: ["mysticism"], unlocks: [] }),
    node({ id: "games_recreation", name: "Games Of Nations", era: "classical", cost: 200, description: "Nations turn competition into grand public games. The first nation to complete this founds the Games of Nations, and the confirmed host city can raise a Grand Stadium to hold the contests. Grants +2 permanent Happiness.", prerequisites: ["state_workforce"], unlocks: [{ type: "policySlot", value: "culture" }], effects: [{ type: "happinessPerTurnFlat", value: 2 }] }),
    ...
    node({ id: "recorded_history", name: "Recorded History", era: "classical", cost: 128, description: "Archives turn memory into an institution. Written accounts help rulers learn from victories, failures, laws, and lineages. Unlocks Archives: a human-only culture policy granting each city +1 Culture and +1 Science per turn.", prerequisites: ["drama_civics", "political_philosophy"], unlocks: [{ type: "building", value: "library" }] }),
    ...
    node({ id: "civil_service_civics", name: "Civil Service", era: "medieval", cost: 200, description: "Administration becomes a profession instead of a favor. Trained officials help distant cities answer to the same state. Unlocks Civil Administration: a human-only diplomatic policy that halves conquered-city integration time while active.", prerequisites: ["recorded_history"], unlocks: [] }),
    node({ id: "mercenaries", name: "Mercenaries", era: "medieval", cost: 210, description: "War becomes a market as soldiers sell skill to the highest cause. Coin, contracts, and reputation can raise armies quickly. Unlocks Mercenary Contracts: a human-only military policy reducing unit upkeep by 10%, stacking with War Economy. Also unlocks Campaign Logistics: a military policy allowing units to upgrade on any tile while active.", prerequisites: ["feudalism"], unlocks: [] }),
    ...
    node({ id: "diplomatic_service", name: "Diplomatic Service", era: "renaissance", cost: 280, description: "Envoys become professionals who speak for the state. Protocol, letters, and permanent missions make diplomacy a civic craft.", prerequisites: ["guilds"], unlocks: [{ type: "policySlot", value: "diplomatic" }] }),
    node({ id: "exploration", name: "Exploration", era: "renaissance", cost: 285, description: "Curiosity and ambition push officials beyond known maps. Exploration turns discovery into policy, profit, and new claims. Scouts and Scout Boats gain +5 sight radius.", prerequisites: ["medieval_faires"], unlocks: [{ type: "government", value: "merchant_republic" }] }),
    node({ id: HUMANISM_CULTURE_NODE_ID, name: "Humanism", era: "renaissance", cost: 290, description: "Art, history, and human dignity move toward the center of public life. Cities preserve achievement as a source of identity and learn to recognize land archaeological sites. Shipwrecks require Natural History.", prerequisites: ["guilds"], unlocks: [{ type: "building", value: "museum" }, { type: "policySlot", value: "culture" }] }),
    ...
    node({ id: "nationalism", name: "Nationalism", era: "renaissance", cost: 440, description: "Shared language, memory, and symbols forge mass identity. Popular movements, independence struggles, and organized resistance take root.", prerequisites: ["humanism", "mercantilism"], unlocks: [{ type: "unit", value: "rebels" }, { type: "policySlot", value: "ideology" }] }),
    node({ id: ENLIGHTENMENT_CULTURE_NODE_ID, name: "The Enlightenment", era: "industrial", cost: 470, description: "Reason, rights, and public inquiry challenge inherited authority. Knowledge becomes a civic engine for reform and progress. Makes the World Council available.", prerequisites: ["humanism", "diplomatic_service"], unlocks: [{ type: "building", value: "university" }, { type: "diplomacy", value: "world_council" }] }),
    node({ id: "natural_history", name: "Natural History", era: "industrial", cost: 500, description: "Careful observation and classification turn the living world into a shared field of public knowledge. Reveals Shipwrecks for underwater archaeology.", prerequisites: ["enlightenment"], unlocks: [] }),
    node({ id: "opera_ballet", name: "Opera and Ballet", era: "industrial", cost: 510, description: "Grand music, theatre, and disciplined movement bring civic performance to an ambitious new scale. Unlocks Phantom of the Opera: a human-only culture policy granting +1 Culture per functioning culture-producing building.", prerequisites: ["enlightenment"], unlocks: [] }),
    ...
    node({ id: "civil_engineering", name: "Civil Engineering", era: "industrial", cost: 525, description: "Public works become symbols of modern administration. Bridges, districts, and services let cities grow with intent. Unlocks National Infrastructure: a human-only economic policy granting all units +3 movement when movement refreshes and on creation.", prerequisites: ["enlightenment"], unlocks: [] }),
    node({ id: "urbanization", name: "Urbanization", era: "modern", cost: 600, description: "City life becomes the dominant rhythm of society. Planning, housing, and services define the politics of modern growth.", prerequisites: ["civil_engineering"], unlocks: [] }),
    ...
    node({ id: "mobilization", name: "Mobilization", era: "atomic", cost: 780, description: "Industry, citizens, and command networks prepare for total war. The nation learns to move as one vast military machine. Unlocks War Economy: a further 10% unit upkeep reduction, stacking with Mercenary Contracts for 20% total.", prerequisites: ["ideology"], unlocks: [] }),
    node({ id: "cold_war", name: "Cold War", era: "atomic", cost: 820, description: "Rival blocs compete through pressure, research, and influence. Diplomacy becomes a contest fought beneath the threshold of war.", prerequisites: ["suffrage", "totalitarianism", "class_struggle"], unlocks: [{ type: "policySlot", value: "diplomatic" }] }),
    ...
    node({ id: "social_media", name: "Social Media", era: "information", cost: 1100, description: "Public life moves through networks of instant attention. Influence spreads through images, messages, and communities without borders.", prerequisites: ["professional_sports", "globalization"], unlocks: [] }),
  ...
  // src/data/centralEasternEuropeanLeaders.ts
  var CENTRAL_EASTERN_EUROPEAN_LEADERS = [
    {
    ...
    },
    {
      ...
      "isDefault": true,
      "name": "Anton\xEDn Z\xE1potock\xFD",
      "nationId": "nation_czechoslovakia",
      "title": "President",
  ...
  var LEADER_ERA_STRATEGY_PROFILES = [
    ...
    { "leaderId": "leader_mikhail_gorbachev", "strategiesByEra": { "ancient": "tallGrowth" } },
    { "leaderId": "leader_suleiman_the_magnificent", "strategiesByEra": { "ancient": "imperialInfrastructure" } },
    { "leaderId": "leader_mohammed_v", "strategiesByEra": { "ancient": "balancedGrowth" } },
    { "leaderId": "leader_tsai_ing_wen", "strategiesByEra": { "ancient": "coastalTechEconomy" } },
    { "leaderId": "leader_sundiata_keita", "strategiesByEra": { "ancient": "frontierExpansion" } },
    { "leaderId": "leader_kublai_khan", "strategiesByEra": { "ancient": "imperialInfrastructure" } },
    { "leaderId": "leader_tokugawa_ieyasu", "strategiesByEra": { "ancient": "defensiveBuilder" } },
    { "leaderId": "leader_anne_bonny", "strategiesByEra": { "ancient": "seaWolf" } },
    { "leaderId": "leader_urho_kekkonen", "strategiesByEra": { "ancient": "balancedGrowth" } },
    { "leaderId": "leader_stephen_harper", "strategiesByEra": { "ancient": "balancedGrowth" } },
    { "leaderId": "leader_vicente_fox", "strategiesByEra": { "ancient": "balancedGrowth" } },
    { "leaderId": "leader_juan_peron", "strategiesByEra": { "ancient": "tallGrowth" } },
    { "leaderId": "leader_petro_poroshenko", "strategiesByEra": { "ancient": "defensiveBuilder" } },
    { "leaderId": "leader_mohammad_reza_pahlavi", "strategiesByEra": { "ancient": "coastalTechEconomy" } },
    { "leaderId": "leader_faisal_i", "strategiesByEra": { "ancient": "tallGrowth" } },
    { "leaderId": "leader_yitzhak_rabin", "strategiesByEra": { "ancient": "defensiveBuilder" } },
    { "leaderId": "leader_mustafa_kemal_ataturk", "strategiesByEra": { "ancient": "imperialInfrastructure" } },
    { "leaderId": "leader_faisal_bin_abdulaziz", "strategiesByEra": { "ancient": "balancedGrowth" } },
    { "leaderId": "leader_thaksin_shinawatra", "strategiesByEra": { "ancient": "tallGrowth" } },
    { "leaderId": "leader_park_chung_hee", "strategiesByEra": { "ancient": "imperialInfrastructure" } },
    { "leaderId": "leader_han_seo_jin", "strategiesByEra": { "ancient": "tallGrowth" } },
    { "leaderId": "leader_sviatlana_tsikhanouskaya", "strategiesByEra": { "ancient": "balancedGrowth" } },
    { "leaderId": "leader_peter_ii_of_yugoslavia", "strategiesByEra": { "ancient": "culturalDominance" } },
    { "leaderId": "leader_vaclav_havel", "strategiesByEra": { "ancient": "culturalDominance" } },
    { leaderId: "leader_alexander_lukashenko", strategiesByEra: { ancient: "defensiveBuilder" } },
    { leaderId: "leader_josip_broz_tito", strategiesByEra: { ancient: "balancedGrowth", industrial: "defensiveBuilder" } },
    { leaderId: "leader_antonin_zapotocky", strategiesByEra: { ancient: "tallGrowth", industrial: "defensiveBuilder" } },
